...
export function CreateProjectTask({ project, currentUserId, tasks, onCreate, getToken, onError }) {
    ...
    return <form className={styles.taskForm} onSubmit={submit}>
        ...
        <div className={styles.formGrid}><div className={styles.staticField}><span>Status</span><strong>To do</strong></div><TaskSelectField label="Priority" value={priority} onChange={setPriority} options={[{ value: 'low', label: 'Low' }, { value: 'medium', label: 'Medium' }, { value: 'high', label: 'High' }]} /><TaskDateField label="Due date" value={dueDate} onChange={setDueDate} /></div>
        <div className={styles.assignmentGrid}><MemberPicker members={project.memberships} selectedIds={assigneeIds} onChange={changeAssignees} /><TaskTagPicker tags={project.tags} selectedIds={tagIds} onChange={setTagIds} disabled={Boolean(generating)} /></div>
...
export function ProjectTags({ project, getToken, onChanged, onError }) {
    ...
    return <section className={styles.collapsibleTool}><button className={styles.toolToggle} type="button" aria-expanded={expanded} aria-controls="project-shared-tags" onClick={() => setExpanded((value) => !value)}><span><b>Shared tags</b><small>{project.tags.length} {project.tags.length === 1 ? 'tag' : 'tags'} available to this project</small></span>{expanded ? <CaretDownIcon aria-hidden="true" size={18} /> : <CaretRightIcon aria-hidden="true" size={18} />}</button><div id="project-shared-tags" className={styles.tagManager} hidden={!expanded}><div>{project.tags.map((tag) => <span key={tag.id}>{tag.name}<button aria-label={`Delete ${tag.name}`} onClick={() => setDeleteTarget(tag)}><XIcon size={13} /></button></span>)}</div><form onSubmit={add}><input maxLength="24" value={name} onChange={(event) => setName(event.target.value)} placeholder="New project tag" aria-label="New project tag" /><TaskSelectField label="New tag color" hideLabel compact value={color} onChange={setColor} options={['slate','red','orange','yellow','green','teal','blue','purple'].map((value) => ({ value, label: value }))} /><button disabled={!name.trim()}>Add tag</button></form></div>{deleteTarget && <ConfirmDialog title="Delete shared tag?" confirmLabel="Delete tag" busyLabel="Deleting…" busy={deleting} onCancel={() => setDeleteTarget(null)} onConfirm={remove}>Delete “{deleteTarget.name}”? It will be removed from every task in this project.</ConfirmDialog>}</section>;
}
