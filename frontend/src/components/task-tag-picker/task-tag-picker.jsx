...
export default function TaskTagPicker({ tags = [], selectedIds = [], onChange, onCreateTag, disabled = false, compact = false }) {
    ...
    return <section className={`${styles.root} ${compact ? styles.compact : ''}`} aria-label="Tags">
        <div className={styles.header}><h3>Tags</h3>{onCreateTag && !disabled && <button type="button" className={styles.addTrigger} aria-label="Create tag" aria-expanded={editorOpen} onClick={() => editorOpen ? closeEditor() : openEditor()}>{editorOpen ? <XIcon size={16} /> : <PlusIcon size={17} />}</button>}</div>
        ...
        {editorOpen && <div className={styles.editor}><input ref={input} aria-label="New tag name" maxLength="24" value={name} placeholder="Tag name" onChange={(event) => setName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); create(); } if (event.key === 'Escape') closeEditor(); }} /><TaskSelectField label="New tag color" hideLabel compact value={color} onChange={setColor} options={colors.map((value) => ({ value, label: value }))} /><button type="button" disabled={creating || !name.trim()} onClick={create}>{creating ? 'Adding…' : 'Add'}</button></div>}
