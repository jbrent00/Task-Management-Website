...
function TaskAssignmentFields({ projects = [], tags, projectId, tagIds, onProjectChange, onTagIdsChange, onCreateTag, showProject = true, members = [], assigneeId = null, onAssigneeChange, allowUnassigned = true }) {
    ...
    return <div className={styles.assignments}>
        ...
        {onAssigneeChange && <label className={styles.field}>Assignee<select value={assigneeId ?? ''} onChange={(event) => onAssigneeChange(event.target.value || null)}>{allowUnassigned && <option value="">Unassigned</option>}{members.filter((member) => member.role !== 'viewer').map((member) => <option key={member.userId} value={member.userId}>{[member.user?.fname, member.user?.lname].filter(Boolean).join(' ') || member.user?.primaryEmail || 'Member'}</option>)}</select></label>}
