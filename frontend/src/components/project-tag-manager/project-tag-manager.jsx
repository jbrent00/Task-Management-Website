...
function ProjectTagManager({ projects, tags, projectCounts, onCreateProject, onUpdateProject, onDeleteProject, onUpdateTag, onDeleteTag }) {
    ...
    const close = () => { setMode(null); setError(''); setEditingProject(null); setEditingTag(null); };
    const create = async (event) => {
        event.preventDefault();
        try { await onCreateProject(title, description); setTitle(''); setDescription(''); close(); }
        catch { setError('Could not save the project. Its name may already be in use.'); }
    };
    const saveProject = async (event) => {
        event.preventDefault();
        try { await onUpdateProject(editingProject.id, title, description); setEditingProject(null); setError(''); }
        catch { setError('Could not update that project. Its name may already be in use.'); }
    };
    const saveTag = async (event) => {
        event.preventDefault();
        try { await onUpdateTag(editingTag.id, tagName, tagColor); setEditingTag(null); setError(''); }
        catch { setError('Could not update that tag. Tag names must be unique and the color must be from the palette.'); }
    };
    const confirmDelete = async () => {
        try {
            if (deleteTarget.type === 'project') await onDeleteProject(deleteTarget.item.id);
            else await onDeleteTag(deleteTarget.item.id);
            setDeleteTarget(null);
        } catch { setError(`Could not delete that ${deleteTarget.type}.`); setDeleteTarget(null); }
    };
    const startProjectEdit = (project) => { setTitle(project.title); setDescription(project.description ?? ''); setEditingProject(project); setError(''); };
    ...
    return <div className={styles.manager}>
        ...
        {editingProject && <div className={styles.dialogBackdrop}><form className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="edit-project-title" onSubmit={saveProject}><h2 id="edit-project-title">Edit project</h2><label>Project name<input value={title} maxLength="100" onChange={(event) => setTitle(event.target.value)} required autoFocus /></label><label>Description<textarea value={description} maxLength="500" onChange={(event) => setDescription(event.target.value)} /></label><div className={styles.dialogActions}><button type="button" onClick={() => setEditingProject(null)}>Cancel</button><button type="submit">Save project</button></div></form></div>}
