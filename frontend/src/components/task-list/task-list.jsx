...
function TaskList({ tasks, status, allTasks, setAllTasks, loading, isManualOrder, isFiltered, projects, tags, onCreateTag }) {
    ...
    return (
        <section className={`${styles.taskList} ${styles[className]}`} aria-label={`${label} tasks`}>
            ...
            {loading ? <p className={styles.loadingState}>Loading tasks...</p> : tasks.length === 0 ? <p className={styles.emptyState}>{isFiltered ? 'No tasks match the current view.' : emptyMessage}</p> : tasks.map((task, index) => (
