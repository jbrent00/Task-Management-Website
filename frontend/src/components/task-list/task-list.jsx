...
function TaskList({ tasks, status, allTasks, setAllTasks, loading, isManualOrder, isFiltered, selectedTab, projects, tags, members = [], projectMode = false, inlineChecklist = false, onCreateTag, onNotify, onOpenDetails, children }) {
    const statusDetails = {
                ...
                <Draggable key={task.id} draggableId={String(task.id)} index={index} isDragDisabled={!isManualOrder || (!projectMode && Boolean(task.projectId)) || task.capabilities?.canReorder === false}>
                    {(provided) => (
                        <div className={styles.draggableItem} ref={provided.innerRef} {...provided.draggableProps}>
                            <TaskCard task={task} allTasks={allTasks} setAllTasks={setAllTasks} projects={projects} tags={tags} members={members} projectMode={projectMode} inlineChecklist={inlineChecklist} onCreateTag={onCreateTag} onNotify={onNotify} onOpenDetails={onOpenDetails} dragHandleProps={provided.dragHandleProps} isDragEnabled={isManualOrder && (projectMode || !task.projectId) && task.capabilities?.canReorder !== false} />
                        </div>
                    )}
                </Draggable>
            ))}
