...
export default async function createTask(req: Request, res: Response) {
    ...
    const { priority, orderIndex, tagIds = [], checklistItems = [], status = 'todo' } = req.body;
    ...
    if (!title || description === undefined || !isTaskPriority(priority) || !isTaskStatus(status) || dueDate === undefined || !isNonNegativeInteger(orderIndex) || !isChecklistItemsInput(checklistItems)) {
    ...
    try {
        const task = await prisma.task.create({
            data: {
                title, description, createdById: userId, priority, status, dueDate, orderIndex, completedAt: status === 'completed' ? new Date() : null,
